...
function AttendancePage() {
  ...
  return <section className="mx-auto max-w-7xl"><PageHeader title="Attendance" description="Track today's headcount and attendance trends." /><div className="grid gap-4 sm:grid-cols-3"><KpiCard label="Present" value={String(present)} icon={UserCheck} /><KpiCard label="Remote" value={String(remote)} icon={CalendarCheck} /><KpiCard label="Late check-ins" value={String(attendance.filter((record) => record.status === "late").length)} icon={Clock3} /></div><div className="surface-card mt-5 p-5"><p className="font-semibold">Today's attendance</p><p className="mt-2 text-sm text-muted-foreground">Check-in records are ready for manager review.</p></div></section>;
}