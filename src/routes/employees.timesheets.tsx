...
function TimesheetsPage() {
  ...
  return <section className="mx-auto max-w-7xl"><PageHeader title="Timesheets" description="Review submitted hours and billable utilization by week." /><div className="grid gap-4 sm:grid-cols-3"><KpiCard title="Billable hours" value={`${billable}h`} icon={Timer} /><KpiCard title="Weeks tracked" value={String(timesheetWeeklyHours.length)} icon={CalendarClock} /><KpiCard title="Pending review" value="6" icon={CheckCircle2} /></div><div className="surface-card mt-5 p-5"><p className="font-semibold">Weekly hours</p><p className="mt-2 text-sm text-muted-foreground">Timesheet submissions are ready for project and payroll review.</p></div></section>;
}