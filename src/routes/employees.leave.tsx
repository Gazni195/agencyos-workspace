...
function LeavePage() {
  return <section className="mx-auto max-w-7xl"><PageHeader title="Leave management" description="Review requests, balances and upcoming time off." /><div className="grid gap-4 sm:grid-cols-3"><KpiCard label="Pending requests" value={String(leaveRequests.filter((request) => request.status === "pending").length)} icon={Clock3} /><KpiCard label="Approved this month" value={String(leaveRequests.filter((request) => request.status === "approved").length)} icon={CheckCircle2} /><KpiCard label="Upcoming leave" value="12" icon={CalendarDays} /></div><div className="surface-card mt-5 p-5"><p className="font-semibold">Leave requests</p><p className="mt-2 text-sm text-muted-foreground">Requests are ready for approval and team planning.</p></div></section>;
}