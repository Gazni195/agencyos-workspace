...
import { employees, attendance, leaveRequests, timesheets, documents, performance } from "@/data/agency.ts";
...
function EmployeeProfilePage() {
    ...
    return (
      <div className="surface-card flex flex-col items-center gap-3 p-12 text-center">
        ...
        <p className="max-w-sm text-sm text-muted-foreground">
          We couldn't find an employee with id "{employeeId}". They may have been removed from the directory.
        </p>
        <Button asChild variant="outline" className="mt-2">
          <Link to="/employees"><ArrowLeft className="mr-1.5 size-4" /> Back to directory</Link>
        </Button>
      </div>
    );
  }
  ...
  const presentDays = empAttendance.filter((a) => a.status === "present" || a.status === "remote").length;

  return (
    <div>
      <Button asChild variant="ghost" size="sm" className="mb-4 -ml-2 gap-1.5 text-muted-foreground">
        <Link to="/employees"><ArrowLeft className="size-4" /> Back to directory</Link>
      </Button>
      ...
      <div className="surface-card mb-6 flex flex-wrap items-center justify-between gap-4 p-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" className="gap-1.5" asChild>
            <a href={`mailto:${employee.email}`}><Mail className="size-4" /> Email</a>
          </Button>
          <Button variant="outline" size="sm" className="gap-1.5" asChild>
            <a href={`tel:${employee.phone}`}><Phone className="size-4" /> Call</a>
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="surface-card p-5 lg:col-span-2">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <InfoRow icon={DollarSign} label="Salary" value={`$${employee.salary.toLocaleString()}/yr`} />
          ...
          <div className="mt-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {employee.skills.length === 0 && <span className="text-sm text-muted-foreground">No skills on file.</span>}
        ...
        <div className="surface-card p-5">
          ...
          {empReview ? (
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">Cycle: {empReview.cycle} · Reviewer: {empReview.reviewer}</p>
              <div>
                <div className="mb-1 flex justify-between text-xs text-muted-foreground">
                  <span>Goals met</span><span>{empReview.goalsMet}/{empReview.goalsTotal}</span>
                </div>
                <Progress value={(empReview.goalsMet / (empReview.goalsTotal || 1)) * 100} className="h-1.5" />
              </div>
            </div>
          ) : (
          ...
          <div className="mt-4 space-y-2">
            {empGoals.map((g) => (
              <div key={g.id}>
                <div className="mb-1 flex justify-between text-xs text-muted-foreground">
                  <span className="truncate pr-2">{g.title}</span><span>{g.progress}%</span>
                </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="surface-card p-5">
          ...
          <p className="mb-3 text-xs text-muted-foreground">{presentDays} present/remote day(s) on record</p>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow><TableHead>Date</TableHead><TableHead>In</TableHead><TableHead>Out</TableHead><TableHead>Status</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {empAttendance.map((a) => (
                  <TableRow key={a.id}>
                ...
                {empAttendance.length === 0 && (
                  <TableRow><TableCell colSpan={4} className="py-6 text-center text-sm text-muted-foreground">No attendance records.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
        ...
        <div className="surface-card p-5">
          ...
          <p className="mb-3 text-sm text-foreground">{employee.leaveBalance} day(s) remaining this cycle</p>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow><TableHead>Type</TableHead><TableHead>Dates</TableHead><TableHead>Days</TableHead><TableHead>Status</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {empLeave.map((l) => (
                  <TableRow key={l.id}>
                    <TableCell>{l.type}</TableCell>
                    <TableCell className="text-muted-foreground">{l.from} → {l.to}</TableCell>
                    <TableCell>{l.days}</TableCell>
                    <TableCell><StatusBadge status={l.status} /></TableCell>
                  </TableRow>
                ))}
                {empLeave.length === 0 && (
                  <TableRow><TableCell colSpan={4} className="py-6 text-center text-sm text-muted-foreground">No leave history.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
        ...
        <div className="surface-card p-5">
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow><TableHead>Date</TableHead><TableHead>Project</TableHead><TableHead>Hours</TableHead><TableHead>Status</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {empTimesheets.map((t) => (
                  <TableRow key={t.id}>
                ...
                {empTimesheets.length === 0 && (
                  <TableRow><TableCell colSpan={4} className="py-6 text-center text-sm text-muted-foreground">No timesheet entries.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
        ...
        <div className="surface-card p-5">
          ...
          <div className="space-y-2">
            ...
            {empDocuments.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">No documents uploaded.</p>}
          </div>
        </div>
      </div>
    </div>
