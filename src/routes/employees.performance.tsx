...
function PerformancePage() {
  ...
  return <section className="mx-auto max-w-7xl"><PageHeader title="Performance" description="Keep goals, reviews and development conversations moving." /><div className="grid gap-4 sm:grid-cols-3"><KpiCard label="Active goals" value={String(employeeGoals.length)} icon={Target} /><KpiCard label="Average progress" value={`${average}%`} icon={Gauge} /><KpiCard label="Reviews due" value="8" icon={Award} /></div><div className="surface-card mt-5 p-5"><p className="font-semibold">Team performance</p><p className="mt-2 text-sm text-muted-foreground">Goal progress and review cycles are ready for manager updates.</p></div></section>;
}