...
function PayrollPage() {
  ...
  return <section className="mx-auto max-w-7xl"><PageHeader title="Payroll" description="Manage monthly payroll summaries and payslips." /><div className="grid gap-4 sm:grid-cols-3"><KpiCard label="Net payroll" value={money(total)} icon={BadgeDollarSign} /><KpiCard label="Employees" value={String(payroll.length)} icon={Receipt} /><KpiCard label="Pay date" value="Aug 30" icon={CalendarDays} /></div><div className="surface-card mt-5 p-5"><p className="font-semibold">August payroll run</p><p className="mt-2 text-sm text-muted-foreground">Payroll summaries and payslips are ready for review.</p></div></section>;
}