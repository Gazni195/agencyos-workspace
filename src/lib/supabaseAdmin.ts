...
if (!url || !serviceRoleKey) {
  throw new Error(
    "Missing SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY server env vars. Set them in .env (never prefix with VITE_ — that would bundle the secret key into client JS).",
  );
}
